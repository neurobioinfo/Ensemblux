...
source(paste0(home_dir,'/gt/scripts/ensemblexing/functions.R'))
## load library
# .libPaths(r_lib_path)
...
par_doublet_Demuxlet_threshold= args[13]
par_doublet_Demuxlet_no_threshold= args[14]
...
print(paste0("par_doublet_Demuxlet_threshold=",par_doublet_Demuxlet_threshold))
print(paste0("par_doublet_Demuxlet_no_threshold=",par_doublet_Demuxlet_no_threshold))
...
par_output_dir<- paste0(output_dir,"/ensemblex_gt")
## Demuxalot
par_demuxalot<- paste0(output_dir,"/demuxalot/Demuxalot_result.csv")
## Demuxlet
par_demuxlet<- paste0(output_dir,"/demuxlet/outs.best")
## Souporcell
par_souporcell<- paste0(output_dir,"/souporcell/clusters.tsv")
## Vireo
par_vireo<- paste0(output_dir,"/vireo_gt/donor_ids.tsv")
...
if (tolower(par_merge_constituents)=="yes"){
    ...
    dim(vireo) ## eventually remove this
    souporcell <- read.delim(par_souporcell, header = T, sep = "\t")
    dim(souporcell) ## eventually remove this
    freemuxlet <-read.delim(par_demuxlet, header = T, sep = "\t")
    dim(freemuxlet) ## eventually remove this
    demuxalot <- read.delim(par_demuxalot, header = T, sep = ",", check.names=FALSE)
    colnames(demuxalot)[1] <- "X"
    dim(demuxalot) ## eventually remove this
    ...
    temp <- freemuxlet[duplicated(freemuxlet$BARCODE),]
        if (nrow(temp) == 0){
            print("Duplicated barcodes in Demuxlet resolved; recommended to investigate initial reason for barcode duplication.")
            print(paste0("Observed ", nrow(freemuxlet), " unique barcodes in Demuxlet output."))
        }
    } else {
        print("No duplicated barcodes observed in Demuxlet output.")
        print(paste0("Observed ", nrow(freemuxlet), " unique barcodes in Demuxlet output."))
    }
    ...
    if (nrow(vireo) == nrow(freemuxlet) &
        ...
        nrow(vireo) == nrow(souporcell)) {
            ...
            message(paste0("WARNING: Demultiplexing tools have reported different number of cells; Vireo: ", nrow(vireo), "; Demuxlet: ", nrow(freemuxlet), "; Demuxalot: ", nrow(demuxalot), "; Souporcell: ", nrow(souporcell), "."))
        }

    #### Make sure that the Sample IDs from each output file have the same structure. Here, we will replace "-" or "." with "_"
    ### Vireo
    ## fix potential "-"
    ...
    ## Check the intersect of unique Sample IDs to make sure they are matching
    vireo_samples <- vireo$best_singlet
    demuxalot_samples <- colnames(demuxalot)[c(2:(par_sample_size + 1))]
    freemuxlet_samples <- freemuxlet$SNG.BEST.GUESS
    Sample_intersect <- Reduce(intersect, list(vireo_samples,demuxalot_samples,freemuxlet_samples))

    ## print message
    if (length(Sample_intersect) == 0){
        message("WARNING: Sample IDs may differ between constituent demultiplexing tools outputs.")
    } else {
        message("Identified matching Sample IDs betweeen constituent demultiplexing tool outputs.")
    }
    ...
    ## Vireo prep
    vireo <- vireo_prep(vireo)  

    ## Demuxlet prep
    freemuxlet <- freemuxlet_prep(freemuxlet)

    ## Souporcell prep
    souporcell <- souporcell_prep(souporcell, demuxalot, vireo, freemuxlet)

    ## Merge results and calculate consensus 
    merge_df <-  merge_concensus(vireo,souporcell, freemuxlet, demuxalot)

    ## Ensure that if outputs do not having matching droplet counts, the missing droplets are unassigned. This occurs with Demuxlet/freemuxlet
    ...
    merge_df$demuxlet_best_assignment[is.na(merge_df$demuxlet_best_assignment)] <- "unassigned"
