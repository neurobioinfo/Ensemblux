...
freemuxlet_prep <- function(freemuxlet){
    ...
    ## Prepare Freemuxlet sample assignment
    ...
    ## Make the sample labels match with Demuxalot
    # Fix best guess
    freemuxlet$freemuxlet_best <- paste0("CLUST", freemuxlet$freemuxlet_best)
    freemuxlet$freemuxlet_best[freemuxlet$freemuxlet_best == "CLUSTdoublet"] <- "doublet"

    # Fix sample
    freemuxlet$freemuxlet_sample <- paste0("CLUST", freemuxlet$freemuxlet_sample)
    freemuxlet$freemuxlet_sample[freemuxlet$freemuxlet_sample == "CLUSTdoublet"] <- "doublet"
    freemuxlet$freemuxlet_sample[freemuxlet$freemuxlet_sample == "CLUSTunassigned"] <- "unassigned"

    ## Print number of retained droplets
    message(paste0("Retained ", nrow(freemuxlet), " droplets during Freemuxlet preparation."))
    freemuxlet
}

souporcell_prep <- function(souporcell, demuxalot, freemuxlet){
    
    message("Matching Souporcell Sample IDs to Demuxalot and Freemuxlet.")
    ...
    ## Freemuxlet
    freemuxlet_for_soup <- freemuxlet[,c(2,21)]
    nrow(freemuxlet_for_soup) == nrow(souporcell)
    ## Merge Souporcell and freemuxlet
    merge_soup_freemuxlet <- merge(freemuxlet_for_soup, souporcell, by.x = "BARCODE", by.y = "barcode")
    nrow(merge_soup_freemuxlet) == nrow(freemuxlet_for_soup)
    merge_soup_freemuxlet <- merge_soup_freemuxlet[,c(2:4)]
    colnames(merge_soup_freemuxlet) <- c("Sample_ID", "status", "assignment")

    ### Calculate cluster-Sample ID probabilities based on assignments of remaining constituent tools
    bind_df <- rbind(merge_soup_demuxalot, merge_soup_freemuxlet)
    bind_df <- subset(bind_df, status != "doublet")
    bind_df <- subset(bind_df, status != "unassigned")
    bind_df <- subset(bind_df, Sample_ID != "doublet")
    bind_df <- subset(bind_df, Sample_ID != "unassigned")
    ...
    unique(souporcell$souporcell_best)
    unique(souporcell$souporcell_sample)
    ...
    ## Print number of retained droplets
    message(paste0("Retained ", nrow(souporcell), " droplets during Souporcell preparation."))
    souporcell
}

vireo_prep <- function(vireo,souporcell, demuxalot, freemuxlet){
    ## Vireo's sample assignment
    #vireo$vireo_sample <- vireo$donor_id

    message("Matching Vireo Sample IDs to Demuxalot, Freemuxlet, and Souporcell.")

    ## Number of singlet clusters identified by Vireo
    vireo_singlet_cluster <- unique(vireo$donor_id)
    vireo_singlet_cluster <- vireo_singlet_cluster[vireo_singlet_cluster != "doublet"]
    vireo_singlet_cluster <- vireo_singlet_cluster[vireo_singlet_cluster != "unassigned"]

    # Temp column
    vireo$donor_id_temp <- vireo$donor_id
    vireo$donor_id[vireo$donor_id == "unassigned" & vireo$prob_max > vireo$prob_doublet] <- vireo$best_singlet[vireo$donor_id == "unassigned" & vireo$prob_max > vireo$prob_doublet]
    vireo$donor_id[vireo$donor_id == "unassigned" & vireo$prob_max < vireo$prob_doublet] <- "doublet"

    ## Demuxalot
    demuxalot_for_vireo <- demuxalot[,c(1,4)]
    nrow(demuxalot_for_vireo) == nrow(vireo)
    merge_vireo_demuxalot <- merge(demuxalot_for_vireo, vireo, by.x = "X", by.y = "cell")
    nrow(merge_vireo_demuxalot) == nrow(demuxalot_for_vireo)
    merge_vireo_demuxalot <- merge_vireo_demuxalot[,c(2,3)]
    merge_vireo_demuxalot$assignment <- merge_vireo_demuxalot$donor_id
    colnames(merge_vireo_demuxalot) <- c("Sample_ID", "status", "assignment")
    merge_vireo_demuxalot$status <- "singlet"
    merge_vireo_demuxalot$status[merge_vireo_demuxalot$assignment == "unassigned"] <- "unassigned"
    merge_vireo_demuxalot$status[merge_vireo_demuxalot$assignment == "doublet"] <- "doublet"

    ## Freemuxlet
    freemuxlet_for_vireo<- freemuxlet[,c(2,21)]
    nrow(freemuxlet_for_vireo) == nrow(vireo)
    merge_vireo_freemuxlet <- merge(freemuxlet_for_vireo, vireo, by.x = "BARCODE", by.y = "cell")
    nrow(merge_vireo_freemuxlet) == nrow(freemuxlet_for_vireo)
    merge_vireo_freemuxlet <- merge_vireo_freemuxlet[,c(2,3)]
    merge_vireo_freemuxlet$assignment <- merge_vireo_freemuxlet$donor_id
    colnames(merge_vireo_freemuxlet) <- c("Sample_ID", "status", "assignment")
    merge_vireo_freemuxlet$status <- "singlet"
    merge_vireo_freemuxlet$status[merge_vireo_freemuxlet$assignment == "unassigned"] <- "unassigned"
    merge_vireo_freemuxlet$status[merge_vireo_freemuxlet$assignment == "doublet"] <- "doublet"

    ## Souporcell
    souporcell_for_vireo<- souporcell[,c(2,11)]
    nrow(souporcell_for_vireo) == nrow(vireo)
    merge_vireo_souporcell <- merge(souporcell_for_vireo, vireo, by.x = "barcode", by.y = "cell")
    nrow(merge_vireo_souporcell) == nrow(souporcell_for_vireo)
    merge_vireo_souporcell <- merge_vireo_souporcell[,c(2,3)]
    merge_vireo_souporcell$assignment <- merge_vireo_souporcell$donor_id
    colnames(merge_vireo_souporcell) <- c("Sample_ID", "status", "assignment")
    merge_vireo_souporcell$status <- "singlet"
    merge_vireo_souporcell$status[merge_vireo_souporcell$assignment == "unassigned"] <- "unassigned"
    merge_vireo_souporcell$status[merge_vireo_souporcell$assignment == "doublet"] <- "doublet"

    ### Calculate cluster-Sample ID probabilities based on assignments of remaining constituent tools
    bind_df <- rbind(merge_vireo_demuxalot, merge_vireo_freemuxlet, merge_vireo_souporcell)
    bind_df <- subset(bind_df, status != "doublet")
    bind_df <- subset(bind_df, status != "unassigned")
    bind_df <- subset(bind_df, Sample_ID != "doublet")
    bind_df <- subset(bind_df, Sample_ID != "unassigned")

    vireo_assignment = "none"
    Sample_ID = "none"
    Probability = "none"
    data_frame_final <- data.frame(vireo_assignment, Sample_ID, Probability)
    ...
    for(i in unique(bind_df$assignment)){
        ...
        Sample_ID = "none"
        Probability = "none"
        data_frame <- data.frame(vireo_assignment, Sample_ID, Probability)
        ...
        Sample_ID = j
        Probability = prob
        data_frame_temp <- data.frame(vireo_assignment, Sample_ID, Probability)
        data_frame <- rbind(data_frame, data_frame_temp)
        }
        data_frame_final <- rbind(data_frame_final, data_frame)
    }
    ...
    ## take maximum cluster-sample probability
    result <- data_frame_final %>% 
    dplyr::group_by(vireo_assignment) %>%
    dplyr::filter(Probability == max(Probability))

    ## set duplicated clusters to unassigned
    n_occur <- data.frame(table(result$vireo_assignment))
    n_occur <- subset(n_occur, Freq > 1)
    duplicated <- n_occur$Var1
    result$Sample_ID[result$vireo_assignment %in% duplicated] <- "unassigned"

    #temp_df <- data.frame(result)
    result <- result[!duplicated(result$vireo_assignment),]
        
    if(length(result$vireo_assignment) == length(unique(bind_df$assignment)) ){
        result <- data.frame(result)
        result <- result[,c(1:2)]
        colnames(result) <- c("donor_id","vireo_sample")
        vireo <- merge(vireo, result, by = "donor_id", all = T)
    }


    vireo$vireo_sample[vireo$donor_id == 'doublet'] <- 'doublet'
    vireo$vireo_sample[is.na(vireo$vireo_sample)] <- "unassigned"
    unique(vireo$vireo_sample)
    
    vireo$vireo_best <- vireo$vireo_sample
    vireo$vireo_sample[vireo$donor_id_temp == "unassigned"] <- "unassigned"

    vireo <- vireo %>% dplyr::select(-donor_id_temp)

    if(length(unique(result$vireo_sample)) == par_sample_size){
        message("Successfully matched all vireo clusters to Sample ID.")
    } else {
        message(paste0("WARNING: ensemblex failed to match all Vireo clusters to Sample ID. ensemblex will proceed; however, it is recommended to look into the data manually."))
    }
...
merge_concensus <- function(vireo,souporcell, freemuxlet, demuxalot){
  ...
  ## Select important columns from Freemuxlet
  ...
  colnames(freemuxlet) <- c("barcode", "freemuxlet_n_snps",  "freemuxlet_n_reads", "freemuxlet_max_probability",  "freemuxlet_assignment", "freemuxlet_best_assignment", "freemuxlet_DIFF_LLK_SNG_DBL")
  ...
  merge_lim <- dplyr::select(merge_df, c("barcode", "vireo_assignment", "souporcell_assignment", "freemuxlet_assignment", "demuxalot_assignment"))
  ...
  merge_df <- dplyr::select(merge_df, -c("vireo_assignment.y", "souporcell_assignment.y","freemuxlet_assignment.y", "demuxalot_assignment.y"))
  colnames(merge_df) <- c("barcode", "vireo_assignment", "souporcell_assignment", "freemuxlet_assignment", "demuxalot_assignment", "general_consensus","vireo_singlet_probability", "vireo_doublet_probability",         
                            ...
                            "freemuxlet_n_snps", "freemuxlet_n_reads", "freemuxlet_max_probability", "freemuxlet_best_assignment", "freemuxlet_DIFF_LLK_SNG_DBL", "demuxalot_max_probability", "demuxalot_best_assignment",         
                            "demuxalot_doublet_probability")
  merge_df <- dplyr::select(merge_df, c("barcode", "vireo_assignment", "souporcell_assignment", "freemuxlet_assignment", "demuxalot_assignment", "general_consensus", "vireo_best_assignment", "souporcell_best_assignment",
                                        "freemuxlet_best_assignment", "demuxalot_best_assignment", "vireo_singlet_probability", "vireo_doublet_probability", "vireo_n_vars", "vireo_best_doublet", "vireo_doublet_logLikRatio",  
                                        "souporcell_log_probability_singlet", "souporcell_log_probability_doublet", "freemuxlet_n_snps", "freemuxlet_n_reads", "freemuxlet_max_probability", "freemuxlet_DIFF_LLK_SNG_DBL", 
...
BA_weight_consensus <- function(merge_df,par_sample_size,par_output_dir){
  ...
  ARI_vireo_freemuxlet <- adj.rand.index(eval_df$vireo_best_assignment, eval_df$freemuxlet_best_assignment) 
  ...
  ## Freemuxlet
  ARI_freemuxlet_freemuxlet <- adj.rand.index(eval_df$freemuxlet_best_assignment, eval_df$freemuxlet_best_assignment) 
  ARI_freemuxlet_vireo <- adj.rand.index(eval_df$freemuxlet_best_assignment, eval_df$vireo_best_assignment) 
  ARI_freemuxlet_demuxalot <- adj.rand.index(eval_df$freemuxlet_best_assignment, eval_df$demuxalot_best_assignment) 
  ARI_freemuxlet_souporcell <- adj.rand.index(eval_df$freemuxlet_best_assignment, eval_df$souporcell_best_assignment) 
  ## Demuxalot
  ...
  ARI_demuxalot_freemuxlet <- adj.rand.index(eval_df$demuxalot_best_assignment, eval_df$freemuxlet_best_assignment) 
  ...
  ARI_souporcell_freemuxlet <- adj.rand.index(eval_df$souporcell_best_assignment, eval_df$freemuxlet_best_assignment) 
  ...
  tool_1 <- c("Vireo","Vireo","Vireo","Vireo",
              "Freemuxlet", "Freemuxlet", "Freemuxlet", "Freemuxlet",
              "Demuxalot","Demuxalot","Demuxalot","Demuxalot",
              "Souporcell","Souporcell","Souporcell", "Souporcell")
  tool_2 <- c("Vireo", "Freemuxlet", "Demuxalot", "Souporcell",
              "Freemuxlet", "Vireo","Demuxalot","Souporcell",
              "Demuxalot", "Souporcell","Freemuxlet","Vireo",
              "Souporcell","Vireo","Freemuxlet","Demuxalot")
  ARI <- c(ARI_vireo_vireo, ARI_vireo_freemuxlet, ARI_vireo_demuxalot, ARI_vireo_souporcell,
           ARI_freemuxlet_freemuxlet, ARI_freemuxlet_vireo, ARI_freemuxlet_demuxalot, ARI_freemuxlet_souporcell,
           ARI_demuxalot_demuxalot, ARI_demuxalot_souporcell, ARI_demuxalot_freemuxlet, ARI_demuxalot_vireo,
           ARI_souporcell_souporcell, ARI_souporcell_vireo, ARI_souporcell_freemuxlet, ARI_souporcell_demuxalot)
  ...
  #### Compute proxy balanced accuracies for each tool using consensus cells from the remaining tools
  ### Vireo 
  ## Compute consensus cells
  eval_df_ba <- eval_df[eval_df$souporcell_best_assignment == eval_df$freemuxlet_best_assignment &
  ...
  eval_df_ba <- eval_df[eval_df$souporcell_best_assignment == eval_df$vireo_best_assignment &
                          eval_df$souporcell_best_assignment == eval_df$demuxalot_best_assignment &
                          eval_df$souporcell_best_assignment != "unassigned" ,] 
  freemuxlet_n <- nrow(eval_df_ba)
  
  ## Compute balanced accuracy
  eval_df_ba$freemuxlet_eval[eval_df_ba$souporcell_best_assignment == "doublet" & eval_df_ba$souporcell_best_assignment == eval_df_ba$freemuxlet_best_assignment] <- "TN"
  eval_df_ba$freemuxlet_eval[eval_df_ba$souporcell_best_assignment != "doublet"  & eval_df_ba$souporcell_best_assignment == eval_df_ba$freemuxlet_best_assignment] <- "TP"
  eval_df_ba$freemuxlet_eval[eval_df_ba$souporcell_best_assignment != "doublet" & eval_df_ba$souporcell_best_assignment != eval_df_ba$freemuxlet_best_assignment] <- "FP"
  eval_df_ba$freemuxlet_eval[eval_df_ba$souporcell_best_assignment == "doublet" & eval_df_ba$souporcell_best_assignment != eval_df_ba$freemuxlet_best_assignment] <- "FN"
  df <- eval_df_ba
  ...
  freemuxlet_df$tool <- "Freemuxlet"
  
  ### Demuxalot
  ## Compute consensus cells
  eval_df_ba <- eval_df[eval_df$souporcell_best_assignment == eval_df$vireo_best_assignment &
                          eval_df$souporcell_best_assignment == eval_df$freemuxlet_best_assignment &
                          eval_df$souporcell_best_assignment != "unassigned" ,] 
  ...
  eval_df_ba <- eval_df[eval_df$demuxalot_best_assignment == eval_df$vireo_best_assignment &
                          eval_df$demuxalot_best_assignment == eval_df$freemuxlet_best_assignment &
  ...
  Tool <- c("Vireo", "Freemuxlet", "Demuxalot", "Souporcell")
  Balanced_accuracy <- c(vireo_BA, freemuxlet_BA, demuxalot_BA, souporcell_BA)
  n_consensus_droplets <- c(vireo_n, freemuxlet_n, demuxalot_n, souporcell_n)
  ...
  ## Freemuxlet
  eval_df$freemuxlet_weighted_probability <- eval_df$freemuxlet_max_probability*BA_df$BA[BA_df$tool == "Freemuxlet"]
  ## Demuxalot
  ...
  ## Freemuxlet
  Freemuxlet_sample_list <- unique(practice_df$freemuxlet_best_assignment)
  ## Souporcell
  ...
  sample_list <- unlist(append(sample_list,Freemuxlet_sample_list)) 
  ...
  for (i in sample_list_2) {
    ...
    practice_df$doublet <- ifelse(practice_df$freemuxlet_best_assignment == "doublet", practice_df$doublet + practice_df$freemuxlet_weighted_probability, practice_df$doublet+ 0)
    ...
    practice_df$sample <- ifelse(practice_df$freemuxlet_best_assignment == i, practice_df$sample + practice_df$freemuxlet_weighted_probability, practice_df$sample+ 0)
  ...
  ## Clean up dataframe
  result_test <- dplyr::select(result_test, c("barcode", "ensemblex_assignment", "ensemblex_probability", "vireo_assignment", "souporcell_assignment", "freemuxlet_assignment", "demuxalot_assignment", "general_consensus",                 
                                              "vireo_best_assignment", "souporcell_best_assignment", "freemuxlet_best_assignment", "demuxalot_best_assignment", "vireo_singlet_probability", "vireo_doublet_probability",         
                                              "vireo_n_vars", "vireo_best_doublet", "vireo_doublet_logLikRatio", "souporcell_log_probability_singlet", "souporcell_log_probability_doublet", "freemuxlet_n_snps", 
                                              "freemuxlet_n_reads", "freemuxlet_max_probability", "freemuxlet_DIFF_LLK_SNG_DBL", "demuxalot_max_probability", "demuxalot_doublet_probability", "vireo_max_probability",
                                              "vireo_weighted_probability", "freemuxlet_weighted_probability", 
...
graph_based_doublet_detection_estimated_par <- function(result_test, par_expected_doublet_rate, par_output_dir){
    ...
    result_2_lim <- result_2_lim[,c(1, 14, 19, 20, 21, 17, 23, 25, 13 )] #barcode, vireo_doublet_probability,  souporcell_log_probability_doublet, freemuxlet_n_snps, reemuxlet_n_reads, vireo_doublet_logLikRatio, reemuxlet_DIFF_LLK_SNG_DBL, demuxalot_doublet_probability, vireo_singlet_probability (we dont use this for PCA)
    ...
    ## Organize Freemuxlet "freemuxlet_DIFF_LLK_SNG_DBL" in ordered frame
    freemuxlet_doublet_df <- result_2 %>% select("barcode", "freemuxlet_DIFF_LLK_SNG_DBL")
    freemuxlet_doublet_df <- freemuxlet_doublet_df %>% arrange(freemuxlet_DIFF_LLK_SNG_DBL)
    ...
    ## Organize Freemuxlet num snp
    freemuxlet_snp_df <- result_2 %>% select("barcode", "freemuxlet_n_snps")
    freemuxlet_snp_df <- freemuxlet_snp_df %>% arrange(desc(freemuxlet_n_snps))
    
    ## Organize Freemuxlet num reads
    freemuxlet_reads_df <- result_2 %>% select("barcode", "freemuxlet_n_reads")
    freemuxlet_reads_df <- freemuxlet_reads_df %>% arrange(desc(freemuxlet_n_reads))
...
graph_based_doublet_detection_par_sweep <- function(result_test, par_expected_doublet_rate, par_output_dir){
    ...
    ### Perform principal component analysis with select variables
    result_2_lim <- result_2
    result_2_lim <- result_2_lim[,c(1, 14, 19, 20, 21, 17, 23, 25, 13 )] #barcode, vireo_doublet_probability,  souporcell_log_probability_doublet, freemuxlet_n_snps, freemuxlet_n_reads, vireo_doublet_logLikRatio, freemuxlet_DIFF_LLK_SNG_DBL, demuxalot_doublet_probability, vireo_singlet_probability (we dont use this for PCA)
    result_2_lim <- result_2_lim[complete.cases(result_2_lim), ]
    res.pca <- prcomp(result_2_lim[,c(2:8)],scale = T)
...
graph_based_doublet_detection_manual_par <- function(result_test, par_expected_doublet_rate, par_output_dir){
...
ensemble_independent_doublet_detections <- function(result_2, par_output_dir){
  ...
  result_2_demuxalot <- result_2_demuxalot %>% dplyr::select(vireo_best_assignment, freemuxlet_best_assignment, souporcell_best_assignment)
  ...
  result_2_demuxalot$two[result_2_demuxalot$freemuxlet_best_assignment == "doublet"] <- 1
  ...
  result_2_vireo <- result_2_vireo %>% dplyr::select(demuxalot_best_assignment, freemuxlet_best_assignment, souporcell_best_assignment)
  result_2_vireo$one <- 0
  result_2_vireo$one[result_2_vireo$demuxalot_best_assignment == "doublet"] <- 1
  result_2_vireo$two <- 0
  result_2_vireo$two[result_2_vireo$freemuxlet_best_assignment == "doublet"] <- 1
  ...
  result_2_souporcell <- result_2_souporcell %>% dplyr::select(demuxalot_best_assignment, freemuxlet_best_assignment, vireo_best_assignment)
  ...
  result_2_souporcell$two[result_2_souporcell$freemuxlet_best_assignment == "doublet"] <- 1
  ...
  ## Freemuxlet
  result_2_freemuxlet <- result_2[result_2$freemuxlet_assignment == "doublet",]
  ...
  result_2_freemuxlet$tool <- "Freemuxlet"
  ...
  result_2_souporcell$tool <- "Souporcell"
  ## Freemuxlet
  result_2_freemuxlet <- result_2[result_2$freemuxlet_best_assignment == "doublet",]
  result_2_freemuxlet <- result_2_freemuxlet %>% dplyr::select(demuxalot_best_assignment, souporcell_best_assignment, vireo_best_assignment)
  result_2_freemuxlet$one <- 0
  result_2_freemuxlet$one[result_2_freemuxlet$demuxalot_best_assignment == "doublet"] <- 1
  result_2_freemuxlet$two <- 0
  ...
  ## Freemuxlet
  result_2_temp_freemuxlet <- result_2
  result_2_temp_freemuxlet$ensemblex_assignment[result_2_temp_freemuxlet$freemuxlet_assignment == "doublet"] <- "doublet"
  n_freemuxlet <- result_2_temp_freemuxlet[result_2_temp_freemuxlet$ensemblex_assignment == "doublet",] %>% nrow()
  ...
  ## Plot
  df <- data.frame(Tool = c("Demuxalot", "Vireo", "Souporcell", "Freemuxlet"),
                    n_doublets = c(n_demuxalot, n_vireo, n_souporcell, n_freemuxlet))
  ...
  ## Freemuxlet
  result_2_temp_freemuxlet <- result_2
  result_2_temp_freemuxlet$ensemblex_assignment[result_2_temp_freemuxlet$freemuxlet_best_assignment == "doublet"] <- "doublet"
  n_freemuxlet <- result_2_temp_freemuxlet[result_2_temp_freemuxlet$ensemblex_assignment == "doublet",] %>% nrow()
 ...
 ####################################################################################################################################################################################
  ...
  ## If Freemuxlet says doublet do doublet
  if ((tolower(par_doublet_Freemuxlet_threshold))=="yes"){
    message("Labelling all doublets identified by Freemuxlet (threshold) as doublets.")
  ...
  ## If Freemuxlet says doublet do doublet
  if ((tolower(par_doublet_Freemuxlet_no_threshold))=="yes") {
     message("Labelling all doublets identified by Freemuxlet (no threshold) as doublets.")
  result_2$ensemblex_assignment[result_2$freemuxlet_best_assignment == "doublet"] <- "doublet"
  }
  ## If Vireo says doublet do doublet
...
confidence_score <- function(result_2, par_output_dir, par_sample_size){
  ...
  ### Vireo 
  eval_df <- result_2
  eval_df_lim <- subset(eval_df, souporcell_best_assignment == freemuxlet_best_assignment &
  ...
  eval_df_lim$consensus_eval_ROC <- "bad"
  eval_df_lim$consensus_eval_ROC[eval_df_lim$freemuxlet_best_assignment == eval_df_lim$souporcell_best_assignment] <- "good"
  ...
  pos <- nrow(temp_pos)
    ...
    if (pos != 0 & neg != 0){
    if (neg <=(0.01*pos) | pos <=(0.01*neg) ){
        print("Limited droplets obtained to compute Freemuxlet AUC; results may not be reflective of true AUC.")
    } else {
        print("Sufficient droplets obtained to compute Freemuxlet AUC.")
    }
  
  roc_empirical <- rocit(score = log(eval_df_lim$freemuxlet_max_probability), class = eval_df_lim$consensus_eval_ROC, 
                         negref = "bad") 
  print(summary(roc_empirical))
  ...
  freemuxlet_AUC_singlet <- freemuxlet_AUC
    ...
    print(paste0("Insufficient droplets to compute Freemuxlet AUC. Observed ", neg, " incorrectly classified droplets and ", pos, " correctly classified droplets. Setting Freemuxlet AUC as 0.5 for confidence score computation."))
  ...
  eval_df <- result_2
  eval_df_lim <- subset(eval_df, souporcell_best_assignment == vireo_best_assignment &
                          souporcell_best_assignment == freemuxlet_best_assignment &
                          souporcell_best_assignment != "unassigned" &
                          souporcell_best_assignment != "doublet")
  ...
  eval_df_lim <- subset(eval_df, demuxalot_best_assignment == vireo_best_assignment &
                          demuxalot_best_assignment == freemuxlet_best_assignment &
  ...
  ## Freemuxlet
  eval_df$ensemblex_singlet_confidence[eval_df$freemuxlet_assignment != "unassigned" & eval_df$freemuxlet_best_assignment != "doublet" &
                               eval_df$freemuxlet_best_assignment == eval_df$ensemblex_assignment] <- eval_df$ensemblex_singlet_confidence[eval_df$freemuxlet_assignment != "unassigned" & eval_df$freemuxlet_best_assignment != "doublet" &
                                                                                                                     eval_df$freemuxlet_best_assignment == eval_df$ensemblex_assignment] + freemuxlet_AUC_singlet
  ...
  eval_df$ensemblex_singlet_confidence[eval_df$vireo_best_assignment == eval_df$demuxalot_best_assignment &
                               eval_df$vireo_best_assignment == eval_df$freemuxlet_best_assignment &
  ...
  eval_df <- dplyr::select(eval_df, c("barcode", "ensemblex_assignment","ensemblex_best_assignment", "ensemblex_probability", "ensemblex_singlet_confidence" , "vireo_assignment","souporcell_assignment","freemuxlet_assignment","demuxalot_assignment","general_consensus" ,                
                                        "vireo_best_assignment","souporcell_best_assignment","freemuxlet_best_assignment","demuxalot_best_assignment","vireo_singlet_probability","vireo_doublet_probability","vireo_n_vars","vireo_best_doublet",                
                                        "vireo_doublet_logLikRatio","souporcell_log_probability_singlet", "souporcell_log_probability_doublet", "freemuxlet_n_snps", "freemuxlet_n_reads","freemuxlet_max_probability", "freemuxlet_DIFF_LLK_SNG_DBL","demuxalot_max_probability"  ,       
                                        "demuxalot_doublet_probability", "vireo_max_probability","vireo_weighted_probability","freemuxlet_weighted_probability","demuxalot_weighted_probability" ,  
